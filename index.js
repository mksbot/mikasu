...
const +18 = JSON.parse(fs.readFileSync('./database/json/+18.json'))
...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:MIKASU\n' // Nama kamu
            + 'ORG:MIKA;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=5528981227647:+55 28981227647\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = '-','=','_'
blocked = []
limitawal = '999999999'
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `QUE PENA QUE SAIU @${num.split('@')[0]} VAI FAZER FALTA 🌀😰😚`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					mod: '[🌀] ESTE PEDIDO É ESPECÍFICO PARA USUARIO MOD MIKASU BOT*',
					benned: 'Você para a banda, por favor, contate o proprietário para abrir sua banda',
					ownerG: '[🌀] MIKASU? Este é um recurso especial para o MIKASU ❌',
					ownerB: '[🌀] MIKASU? Este é um recurso especial para o MIKASU ❌',
					userB: `──「 LISTA 」──\nOlá Kak !\nDesculpe, irmã. Você não está registrado como amigo de MIKASU. Registre-se para fazer amizade com o bot MIKASU por meio, \n\nCommand : ${prefix}daftar nama|idade\nExemplo : ${prefix}daftar MIKASU|17\n\n──「 MIKASU BOT 」──`,
			...
			const ownerNumber = ["5528981227647@s.whatsapp.net"] // replace this with your number
			const mod = [ownerNumber,"5528981227647@s.whatsapp.net"]//ubah nomor lo
			const adminbotnumber = ["5528981227647@s.whatsapp.net"]//ubah nomor lo
			const frendsowner = ["5528981227647@s.whatsapp.net"]//ubah nomor lo
			const premium = ["5528981227647@s.whatsapp.net"] 
			...
			const is+18 = isGroup ? +18.includes(from) : true
...
if (text.includes("placa"))
  ...
  conn.sendMessage(id, hasil, MessageType.text); 
	...
	if (messagesC.includes("fdp")){
		...
		if (messagesC.includes("ai ai ai")){
			client.updatePresence(from, Presence.composing)
		...
		if (messagesC.includes("bot")){
			...
			if (messagesC.includes("bom dia bot")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/bahc.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
	
				if (messagesC.includes("MIKASU")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/MIKASU.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
	
	        if (messagesC.includes("grupo")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/grupo.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
	
	if (messagesC.includes("videos adultos")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/trolei.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
	
			if (messagesC.includes("boa tarde bot")){
			client.updatePresence(from, Presence.composing)
			tujuh = fs.readFileSync('./assets/Dbz.mp3');
            client.sendMessage(from, tujuh, MessageType.audio, {quoted: mek, mimetype: 'audio/mp4', ptt:true})
	}
	        if (messagesC.includes("boa noite bot")){
			client.updatePresence(from, Presence.composing)
          ...
          case 'snack':
               ...
               if (anu.error) return reply(anu.error)
				...
				case 'modo+18':
					...
					if (args.length < 1) return reply('Hmmmm')
					if ((args[0]) === 'on') {
						if (is+18) return reply('O modo +18 já está ativo')
						+18.push(from)
						fs.writeFileSync('./database/json/+18.json', JSON.stringify(+18))
...
//@darkYT					
    ...
    break
				...
				case 'rize':
					...
					break 
				case 'mia':
				  	if (!is+18) return reply(' *Ative o modo +18, para ativar use: .modo+18 on* ')
					reply(mess.wait)
		...
		case 'happymod':
			...
			break
				...
				case '+18boquete':
					...
					reply(mess.wait)
					anu = await fetchJson(`https://tobz-api.herokuapp.com/api/+18blowjob}`, {method: 'get'})
					naru = JSON.parse(JSON.stringify(anu));
					to =  naru[Math.floor(Math.random() * naru.length)];
					nye = await getBuffer(to)
					client.sendMessage(from, nye, image, { caption: 'naruto!!', quoted: mek })
...
break
			    ...
			    case '+18blowjob':
				    try {
						if (!is+18) return reply('❌ *FALSO* ❌')
						res = await fetchJson(`https://tobz-api.herokuapp.com/api/+18blowjob`, {method: 'get'})
			    ...
			    case '+18neko':
				    try {
						if (!is+18) return reply('❌ *FALSO* ❌')
						res = await fetchJson(`https://tobz-api.herokuapp.com/api/+18neko`, {method: 'get'})
						buffer = await getBuffer(res.result)
				...
				case '+18trap':
				    try {
						if (!is+18) return reply('❌ *FALSO* ❌')
						res = await fetchJson(`https://tobz-api.herokuapp.com/api/+18trap`, {method: 'get'})
						buffer = await getBuffer(res.result)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'ni Anjim'})
					} catch (e) {
						console.log(`Error :`, color(e,'red'))
				...
				case 'wibu':
                   ...
                   break
					case 'totaluser':
					if (!isOwner) return reply(mess.only.ownerB)    
					teks = `\`\`\`╭────*「 *TOTAL DE USUÁRIOS MIKASU BOT 👑* 」\n\`\`\``
					no = 0
...
// recursos anime, random @MIKASU domina vida

                case 'anjing':
                   if (!isGroup) return reply(ind.groupo())
                   if (!is+18) return reply(ind.+18off())
          ...
          reply('Espere..')
                ...
                case 'blowjob':
					ranp = getRandom('.gif')
					rano = getRandom('.webp')
					anu = await fetchJson(`https://tobz-api.herokuapp.com/api/+18blowjob?apikey=${TobzKey}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					exec(`wget ${anu.result} -O ${ranp} && ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=15 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${rano}`, (err) => {
						fs.unlinkSync(ranp)
						if (err) return reply(ind.stikga())
				...
				case 'walpaperanime':
					...
					break
					case 'dado':    
					if (!isPremium) return reply('Você não é um Membro Premium, entre em contato com o MIKASU ou digite *.Daftarvip* para adquirir o acesso Premium!' ,text, { quoted: mek })
				...
				case 'neko':
					...
					client.sendMessage(from, tod, image, { quoted: mek, caption: '*╭────*「 *ADMINBOT MIKASU ✨* 」\n*│+ wa.me/5528981227647*╰──────*「 *MIKASU* 」*────*\n\n*_SE QUER SER ADMIN DO BOT MIKASU_*\n*_Tipo /iklan_*' })
					break
				case 'iklan':
				...
				case 'canal':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '️*canal do MIKASU:*\n\n https://youtube.com/channel/UCpB3qh2Sp3K23s9a2Q-Gf-g'})
					break
				case '+18loli1':
					memein = await kagApi.memeindo()
           ...
           case 'shitlist':
               ...
               break
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* MIKASU\n* *WPP:* wa.me/+5528981227647\n*\n\n\nEspero que tenham gostado do bot 💥🚩️'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case 'loli':
					...
					break
                                case 'lolih':
                                        ...
                                        break
				case '+18loli':
					if (!is+18) return reply('*COMANDO SO FUNCIONA NO PV AMIGO.*')
					loli.get+18Loli(async (err, res) => {
        ...
        case 'promover':
				...
				default:
					if (isGroup && isSimi && budy != undefined) {
						...
						console.log(color('[ERROR]','red'), 'fui tapeado', color(sender.split('@')[0]))
					}
                           }
		} catch (e) {
			console.log('Error : %s', color(e, 'red'))
